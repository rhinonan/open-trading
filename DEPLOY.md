...
Open Trading 当前以 **应用容器 + Redis 容器 + 挂载数据卷** 部署。默认对外端口 **3002**。
...
**不要水平扩多个 open-trading 副本**：BullMQ Worker 与 Next 同进程，且业务库为 SQLite 单写者；多实例会抢任务、DB 冲突。Redis 可单独部署。
...
| `REDIS_URL` | BullMQ 队列（Compose 内默认 `redis://redis:6379`；本机 dev `redis://127.0.0.1:6379`） |
